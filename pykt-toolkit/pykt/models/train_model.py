...
def model_forward(model, data, rel=None):
    ...
    #print(f"device in train model:{device}")
    ys, preloss = [], []
    ...
    if model_name in ["rkt"]:
    ...
    elif model_name in ["rakt"]:
        y, sim = model(dcur, rel, train=True)
        ys.append(y[:,1:])
        loss_bce = cal_loss(model, ys, r, rshft, sm, preloss)
        qc_label = model.q_matrix[cq]
        loss_qc = binary_cross_entropy(sim.double() , qc_label.double())
        loss = loss_bce + model.beta * loss_qc
        ...
        y, y2, y3 = model(dcur, dgaps, rel, train=True)
        ys = [y[:,1:], y2, y3]
    ...
    # cal loss
        ...
        ys.append(y[:, 1:])
    elif model_name in que_type_models and model_name not in special_models and model.model_name != "rakt":
        y,loss = model.train_one_step(data, rel)
...
def train_model(model, train_loader, valid_loader, num_epochs, opt, ckpt_path, test_loader=None, test_window_loader=None, save_model=False, data_config=None, fold=None):
    ...
    for i in range(1, num_epochs + 1):
        ...
        for data in train_loader:
            train_step+=1
            if model.model_name in que_type_models and model.model_name not in special_models and model.model_name != "rakt":
                model.model.train()
            else:
                model.train()
            ...
            if model.model_name in ["rkt", "rkt_abl"]:
        ...
        auc, acc, valid_loss = evaluate(model, valid_loader, model.model_name, rel)
        ### atkt 有diff， 以下代码导致的
        ...
        print(f"Epoch: {i}, validauc: {validauc:.4}, validacc: {validacc:.4}, best epoch: {best_epoch}, best auc: {max_auc:.4}, train loss: {loss_mean}, valid loss:{valid_loss}, emb_type: {model.emb_type}, model: {model.model_name}, save_dir: {ckpt_path}")
