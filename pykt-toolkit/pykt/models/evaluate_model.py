...
def cal_loss(model, ys, r, rshft, sm, preloss=[]):
    ...
    if model_name in ["atdkt", "simplekt", "bakt_time", "sparsekt", "bakt_time2"]:
        y = torch.masked_select(ys, sm)
        ...
        y = torch.masked_select(ys, sm)
        t = torch.masked_select(rshft, sm)
        loss = binary_cross_entropy(y.double(), t.double()) + preloss
    elif model_name == "lpkt":
        y = torch.masked_select(ys[0], sm)
        t = torch.masked_select(rshft, sm)
        criterion = nn.BCELoss(reduction='none')        
        loss = criterion(y, t).sum()
    elif (model_name in ["atkt", "atktfix"]+que_type_models) or (model_name in special_models):
        loss = 0.0
...
def evaluate(model, test_loader, model_name, rel, save_path=""):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["dkt_forget", "bakt_time", "bakt_time2"]:
                dcur, dgaps = data
            else:
                dcur = data
            ...
            if model.model_name in que_type_models and model_name not in special_models and model_name != "rakt":
                model.model.eval()
            else:
                model.eval()
            ...
            if model_name in ["atdkt"]:
            ...
            elif model_name in ["rakt", "rkt", "rkt_paper", "rkt_abl"]:
                y, attn = model(dcur, rel)
                y = y[:,1:]
                if q.numel() > 0:
                    c,cshft = q,qshft   #question level 
            elif model_name in ["bakt_time", "bakt_time2"]:
                y = model(dcur, dgaps, rel)
                y = y[:,1:]
                ...
                y = y[:, 1:]
            elif model_name in que_type_models and model_name != "lpkt" and model_name != "rakt":
                y = model.predict_one_step(data, rel)
                c,cshft = q,qshft#question level 
            elif model_name == "dimkt":
                y = model(q.long(),c.long(),sd.long(),qd.long(),r.long(),qshft.long(),cshft.long(),sdshft.long(),qdshft.long())
            # print(f"after y: {y.shape}")
            # cal valid loss
            # valid_loss = cal_loss(model, y, r, rshft, sm, reg_loss)
...
def early_fusion(curhs, model, model_name):
    if model_name in ["dkvmn","skvmn"]:
        ...
        p = p.squeeze(-1)
    elif model_name in ["akt", "simplekt", "bakt_time", "bakt_time2", "sparsekt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:
...
def effective_fusion(df, model, model_name, fusion_type):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "akt", "simplekt", "bakt_time", "bakt_time2", "sparsekt", "saint", "sakt", "hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
    for ui in df:
        # 一题一题处理
        curdf = ui[1]
...
def evaluate_question(model, test_loader, model_name, fusion_type=["early_fusion", "late_fusion"], rel=None, save_path=""):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["dkvmn","skvmn"]:
                ...
                y, h = model(dcurori, dgaps, rel, qtest=True, train=False)
                y = y[:,1:]
